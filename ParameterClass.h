...
private:
    ...
    std::string QuantParam = "biological";                      /* how TGV, TBV and TDD are calculated */
    ...
    /* Genetic Parameters can be based on multiple traits */
    std::vector<double> Var_Additive = std::vector<double>(0);  /* Additive Variance */
    std::vector<double> Var_Dominance = std::vector<double>(0); /* Dominance Variance */
    std::vector<double> Var_Residual = std::vector<double>(0);  /* Residual Variance */
    int numberoftraits = 1;                                     /* Number of quantitiative traits */
    ...
    std::vector <std::string> AtSelectionHas_vec = std::vector<std::string>(0);  /* What animal has at and after selection across traits */
    std::string AtSelectionHas = "phenotype";                   /* What animal has at and after selection */
    std::vector<double> IndexWeights = std::vector<double>(0);  /* Index weight if doing multiple trait */
    ...
    double FemalePropGenotype = 1.0;                            /* Proportion of Females Genotyped */
    std::string FemaleWhoGenotype = "parents";                  /* Who and at what point an animal gets genotyped */
    std::string GenotypePortionofDistribution = "high";         /* Which portion of the distribution animals are genotyped */
    /* Blending G A22 Options */
    std::vector <double> blending_G_A22 = std::vector<double>(0);   /* G*vector[0] + A22*vector[1] */
    /* Phenotyping Strategy Options */
    std::vector <double> MalePropPhenotype_vec = std::vector<double>(0);                /* Proportion of Males Phenotyped */
    std::vector <std::string> MaleWhoPhenotype_vec = std::vector<std::string>(0);       /* Who and at what point an animal gets phenotyped */
    std::vector <double> FemalePropPhenotype_vec = std::vector<double>(0);              /* Proportion of Females Phenotyped */
    std::vector <std::string> FemaleWhoPhenotype_vec = std::vector<std::string>(0);     /* Who and at what point an animal gets phenotyped */
    /* If doing EBV based phenotying which portion of distribution to grab */
    std::vector <std::string> PortionofDistribution_vec = std::vector<std::string>(0);
    /**********************************************************/
    /* To generate particular phenotype and genotype scenario */
    /**********************************************************/
    std::string Interim_EBV = "no";                         /* Generate InterimEBV if doing complicated phenotype scenario */
    std::string ImputationFile = "nofile";                  /* Do you want to do imputation and if so bash script to run imputation program */
public:
    parameters();
    ~parameters();
    /* Start Location */
    ...
    /* how TGV, TBV and TDD are calculated */
    std::string getQuantParam(){return QuantParam;}
    void UpdateQuantParam(std::string temp);
    /* Gamma Shape Lethal */
    ...
    /* Additive Variance */
    const std::vector <double>& get_Var_Additive() const{return Var_Additive;}
    void add_Var_Additive(double x){Var_Additive.push_back(x);}
    void clear_Var_Additive(){Var_Additive.clear();}
    /* Dominance Variance */
    const std::vector <double>& get_Var_Dominance() const{return Var_Dominance;}
    void add_Var_Dominance(double x){Var_Dominance.push_back(x);}
    void clear_Var_Dominance(){Var_Dominance.clear();}
    /* Residual Variance */
    const std::vector <double>& get_Var_Residual() const{return Var_Residual;}
    void add_Var_Residual(double x){Var_Residual.push_back(x);}
    void clear_Var_Residual(){Var_Residual.clear();}
    /* Number of quantitiative traits */
    int getnumbertraits(){return numberoftraits;}
    void Updatenumbertraits(int temp);
    ...
    /* Index weight if doing multiple trait */
    const std::vector <double>& get_IndexWeights() const{return IndexWeights;}
    void add_IndexWeights(double x){IndexWeights.push_back(x);}
    ...
    /* What animal has at and after selection across traits */
    const std::vector < std::string >& get_AtSelectionHas_vec() const{return AtSelectionHas_vec;}
    void add_AtSelectionHas_vec(std::string x){AtSelectionHas_vec.push_back(x);}
    void clear_AtSelectionHas_vec(){AtSelectionHas_vec.clear();}
    ...
    /* Which portion of the distribution animals are genotyped */
    std::string getGenotypePortionofDistribution(){return GenotypePortionofDistribution;}
    void UpdateGenotypePortionofDistribution(std::string temp);
    /* Blending G A22 Options; G*vector[0] + A22*vector[1] */
    const std::vector <double>& get_blending_G_A22() const{return blending_G_A22;}
    void add_Toblending_G_A22(double x){blending_G_A22.push_back(x);}
    void clear_blending_G_A22(){blending_G_A22.clear();}
    /* Proportion of Males Phenotyped */
    const std::vector <double>& get_MalePropPhenotype_vec() const{return MalePropPhenotype_vec;}
    void add_MalePropPhenotype_vec(double x){MalePropPhenotype_vec.push_back(x);}
    /* Who and at what point an animal gets phenotyped */
    const std::vector <std::string>& get_MaleWhoPhenotype_vec() const{return MaleWhoPhenotype_vec;}
    void add_MaleWhoPhenotype_vec(std::string x){MaleWhoPhenotype_vec.push_back(x);}
    /* Proportion of Females Phenotyped */
    const std::vector <double>& get_FemalePropPhenotype_vec() const{return FemalePropPhenotype_vec;}
    void add_FemalePropPhenotype_vec(double x){FemalePropPhenotype_vec.push_back(x);}
    /* Who and at what point an animal gets phenotyped */
    const std::vector <std::string>& get_FemaleWhoPhenotype_vec() const{return FemaleWhoPhenotype_vec;}
    void add_FemaleWhoPhenotype_vec(std::string x){FemaleWhoPhenotype_vec.push_back(x);}
    /* If doing ebv based selection part of distribution that is grabbed */
    const std::vector <std::string>& get_PortionofDistribution_vec() const{return PortionofDistribution_vec;}
    void add_PortionofDistribution_vec(std::string x){PortionofDistribution_vec.push_back(x);}
    /* Generate InterimEBV if doing complicated phenotype scenario */
    std::string getInterim_EBV(){return Interim_EBV;}
    void UpdateInterim_EBV(std::string temp);
    /* Do you want to do imputation and if so bash script to run imputation program */
    std::string getImputationFile(){return ImputationFile;}
    void UpdateImputationFile(std::string temp);
};
