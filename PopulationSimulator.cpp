...
int main()
{
    system("chmod 755 macs");
    system("chmod 755 msformatter");
    ...
    cin >> paramterfile;
    cout << endl;
    string StartSim, Selection, EBV_Calc, Solver, Geno_Inverse, Mating, Culling, SelectionDir,Create_LD_Decay, OutputGeno, Recomb_Distribution, Ne_spec;
    string outputfolder;
    ...
    int outputgeneration, nthread, maxmating;
    double ThresholdMAFQTL, ThresholdMAFMark, LowerThresholdMAFFitnesslethal, LowerThresholdMAFFitnesssublethal, Gamma_Shape, Gamma_Scale, Normal_meanRelDom;
    double Normal_varRelDom, Gamma_Shape_Lethal,Gamma_Scale_Lethal, Normal_meanRelDom_Lethal, Normal_varRelDom_Lethal;
    double Gamma_Shape_SubLethal, Gamma_Scale_SubLethal, Normal_meanRelDom_SubLethal, Normal_varRelDom_SubLethal, PropQTL;
    double Variance_Additiveh2, Variance_Dominanceh2, SireReplacement, DamReplacement, BetaDist_alpha, BetaDist_beta, proppleitropic, genetic_correlation;
    float u;
    ...
    string logfileloc = path + "/" + outputfolder + "/log_file.txt"; string lowfitnesspath = path + "/" + outputfolder + "/Low_Fitness";
    string snpfreqfileloc = path + "/" + outputfolder + "/SNPFreq"; string foundergenofileloc = path + "/" + outputfolder + "/FounderGenotypes";
    string qtl_class_object = path + "/" + outputfolder + "/QTL_new_old_Class"; string Pheno_Pedigree_File = path + "/" + outputfolder + "/Pheno_Pedigree";
    string Pheno_GMatrix_File = path + "/" + outputfolder + "/Pheno_GMatrix"; string Master_DF_File = path + "/" + outputfolder + "/Master_DF";
    string Master_Genotype_File = path + "/" + outputfolder + "/Master_Genotypes"; string BinaryG_Matrix_File = path + "/" + outputfolder + "/G_Matrix";
    string Binarym_Matrix_File = path + "/" + outputfolder + "/m_Matrix"; string Binaryp_Matrix_File = path + "/" + outputfolder + "/p_Matrix";
    string BinaryLinv_Matrix_File = path + "/" + outputfolder + "/Linv_Matrix"; string BinaryGinv_Matrix_File = path + "/" + outputfolder + "/Ginv_Matrix";
    string Marker_Map = path + "/" + outputfolder + "/Marker_Map"; string LD_Decay_File = path + "/" + outputfolder + "/LD_Decay";
    ...
    /* Clear out log_file.txt from previous simulation */
    ...
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); seednumber = atoi(parm[search].c_str());
            logfile << "        - Seed Number:\t\t\t\t\t\t\t\t\t\t\t" << "'" << seednumber<< "'" << endl; break;
        }
        search++;
        if(search >= parm.size()){seednumber = time(0); logfile<< "        - Seed Number:\t\t\t\t\t\t\t\t\t\t\t" <<"'"<<seednumber<<"' (System Clock)"<<endl; break;}
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); nthread = atoi(parm[search].c_str());
            logfile << "        - Number of threads:\t\t\t\t\t\t\t\t\t\t" << "'" << nthread << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            nthread = 1;
            logfile << "        - Number of threads:\t\t\t\t\t\t\t\t\t\t" << "'" << nthread << "' (Default)" << endl; break;
        }
    }
    omp_set_num_threads(nthread);
    logfile << "    - Genome and Marker:" << endl;
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); numchra = atoi(parm[search].c_str()); break;
        }
        search++; if(search > parm.size()){logfile << endl << "Couldn't find 'CHR:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    const int numChr=numchra; logfile<<"        - Number of Chromosomes:\t\t\t\t\t\t\t\t\t"<<"'"<<numChr<<"'"<<endl; float ChrLength [numChr]; search = 0;
    ...
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(timescycled != numChr){logfile << endl << "CHR_LENGTH " << timescycled << " doesn't correspond to CHR number!" << endl; exit (EXIT_FAILURE);}
            break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'CHR_LENGTH:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    /* Check to ensure isn't 0 which can happen if leave a blank when you give one less */
    for(int i = 0; i < numChr; i++){if(ChrLength[i] == 0){logfile << endl << "Incorrect Chromosome Length of 0!" << endl; exit (EXIT_FAILURE);}}
    logfile << "        - Length of Chromosome: " << endl;
    for(int i = 0; i < numChr; i++){logfile << "            \t\t\t\t\t\t\t\t\t\t\t\tChr " << i + 1 << ":'" << ChrLength[i] << "'" << endl;}
    int numMark [numChr];
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(timescycled != numChr){logfile << endl << "NUM_MARK " << timescycled << " doesn't correspond to CHR number!" << endl; exit (EXIT_FAILURE);}
            break;
        }
        search++;
        if(search >= parm.size()){logfile << endl << "Couldn't find 'NUM_MARK: ' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    for(int i = 0; i < numChr; i++){if(numMark[i] == 0){logfile << endl << "Incorrect Number of Marker value of 0!" << endl; exit (EXIT_FAILURE);}}
    logfile << "        - Number of Markers: " << endl;
    for(int i = 0; i < numChr; i++){logfile << "            \t\t\t\t\t\t\t\t\t\t\t\tChr " << i + 1 << ":'" << numMark[i] << "'" << endl;}
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); ThresholdMAFMark = atof(parm[search].c_str());
            logfile << "        - Minor Allele Frequency Threshold for Markers:\t\t\t\t\t\t\t" << "'" << ThresholdMAFMark << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            ThresholdMAFMark = 0.05;
            logfile << "        - Minor Allele Frequency Threshold for Markers:\t\t\t\t\t\t\t" << "'" << ThresholdMAFMark << "' (Default)" << endl; break;
        }
    }
    if(ThresholdMAFMark >= 0.5){logfile << endl << "Marker MAF threshold can't be greater than 0.5! Check parameter file!" << endl; exit (EXIT_FAILURE);}
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); qtla = atoi(parm[search].c_str()); break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'QTL:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    const int numQTL = qtla;
    logfile << "        - Number of Quantitative Trait QTL by Chromosome:\t\t\t\t\t\t" << "'" << numQTL << "'" << endl;
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); ThresholdMAFQTL = atof(parm[search].c_str());
            logfile << "        - Minor Allele Frequency Threshold for Quantitative Trait QTL's:\t\t\t\t" << "'" << ThresholdMAFQTL << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            ThresholdMAFQTL = 0.05;
            logfile << "        - Minor Allele Frequency Threshold for Quantitative Trait QTL's:\t\t\t\t" << "'" << ThresholdMAFQTL << "' (Default)" << endl;
            break;
        }
    }
    if(ThresholdMAFQTL >= 0.5){logfile << endl << "Quantitative MAF threshold can't be greater than 0.5! Check parameter file!" << endl; exit (EXIT_FAILURE);}
    search = 0;
    while(1)
    {
    ...
    if(qtlb > 0){logfile << "        - Number of Lethal Fitness Trait QTL by Chromosome:\t\t\t\t\t\t" << "'" << numFitnessQTLlethal << "'" << endl;}
    if(qtlb == 0){logfile << "        - Number of Lethal Fitness Trait QTL by Chromosome:\t\t\t\t\t\t" << "'" << numFitnessQTLlethal << "' (Default)" << endl;}
    search = 0;
    while(1)
    {
    ...
    if(qtlc > 0){logfile << "        - Number of Sub-Lethal Fitness Trait QTL by Chromosome:\t\t\t\t\t\t" << "'" << qtlc << "'" << endl;}
    if(qtlc == 0){logfile << "        - Number of Sub-Lethal Fitness Trait QTL by Chromosome:\t\t\t\t\t\t" << "'" << qtlc << "' (Default)" << endl;}
    if((numQTL+numFitnessQTLlethal+numFitnessQTLsublethal) > 5000)
    {
        logfile << endl << "Cannot have greater than 5000 QTL! Check parameter file." << endl; exit (EXIT_FAILURE);
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            {
                logfile << endl << "        - Should be two values not one for fitness MAF!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            LowerThresholdMAFFitnesslethal = atof(temp.c_str()); parm[search].erase(0, pos + 1);
            LowerThresholdMAFFitnesssublethal = atof(parm[search].c_str());
            logfile << "        - Upper Minor Allele Frequency Threshold for Fitness Lethal QTL's:\t\t\t\t" << "'";
            logfile << LowerThresholdMAFFitnesslethal << "'" << endl;
            logfile << "        - Upper Minor Allele Frequency Threshold for Fitness Sub Lethal QTL's:\t\t\t\t" << "'";
            logfile << LowerThresholdMAFFitnesssublethal << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            ...
            logfile << "        - Upper Minor Allele Frequency Threshold for Fitness Lethal QTL's:\t\t\t\t" << "'";
            logfile << LowerThresholdMAFFitnesslethal << "' (Default)" << endl; break;
            logfile << "        - Upper Minor Allele Frequency Threshold for Fitness Sub Lethal QTL's:\t\t\t\t" << "'";
            logfile << LowerThresholdMAFFitnesssublethal << "' (Default)" << endl; break;
            break;
        }
    }
    if(LowerThresholdMAFFitnesslethal >= 0.5 || LowerThresholdMAFFitnesssublethal >= 0.5)
    {
        logfile << endl << "Fitness MAF upper threshold can't be greater than 0.5! Check parameter file!" << endl; exit (EXIT_FAILURE);
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); haplo_size = atoi(parm[search].c_str());
            logfile << "        - SNP Haplotype Size:\t\t\t\t\t\t\t\t\t\t" << "'" << haplo_size << "'" << endl; break;
        }
        search++;
        if(search >= parm.size()){haplo_size=50; logfile<< "        - SNP Haplotype Size:\t\t\t\t\t\t\t\t\t\t"<<"'"<<haplo_size<<"' (Default)"<<endl;break;}
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            logfile<< "        - Recombination Position generated from the following distribution:\t\t\t\t" << "'" << Recomb_Distribution << "'" << endl;
            break;
        }
        search++;
        if(search >= parm.size())
    ...
    {
        logfile << endl << "RECOMBINATION (" << Recomb_Distribution << ") didn't equal Uniform or Beta! Check parameter file!" << endl; exit (EXIT_FAILURE);
    }
    const int haplotypesize = haplo_size; logfile << "    - QTL Effects:" << endl;
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for ADD_QUAN!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            Gamma_Shape = atof(temp.c_str()); parm[search].erase(0, pos + 1); Gamma_Scale = atof(parm[search].c_str());
            logfile << "        - Gamma Shape: Additive Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Gamma_Shape << "'" << endl;
            logfile << "        - Gamma Scale: Additive Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Gamma_Scale << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Gamma_Shape = 0.4; Gamma_Scale = 1.66;
            logfile << "        - Gamma Shape: Additive Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Gamma_Shape << "' (Default)" << endl;
            logfile << "        - Gamma Scale: Additive Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Gamma_Scale << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for DOM_QUAN!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos); Normal_meanRelDom = atof(temp.c_str()); parm[search].erase(0, pos + 1);
            Normal_varRelDom = atof(parm[search].c_str());
            logfile << "        - Normal Mean: Dominance Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom << "'" << endl;
            logfile << "        - Normal SD: Dominance Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Normal_meanRelDom = 0.1; Normal_varRelDom = 0.2;
            logfile << "        - Normal Mean: Dominance Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom << "' (Default)" << endl;
            logfile << "        - Normal SD: Dominance Quantitative Trait:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom << "' (Default)" << endl;
            break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for LTHA!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            Gamma_Shape_Lethal = atof(temp.c_str()); parm[search].erase(0, pos + 1); Gamma_Scale_Lethal = atof(parm[search].c_str());
            logfile << "        - Gamma Shape: S value Fitness Lethal:\t\t\t\t\t\t\t\t" << "'" << Gamma_Shape_Lethal << "'" << endl;
            logfile << "        - Gamma Scale: S value Fitness Lethal:\t\t\t\t\t\t\t\t" << "'" << Gamma_Scale_Lethal << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Gamma_Shape_Lethal = 0.3; Gamma_Scale_Lethal = 0.1;
            logfile << "        - Gamma Shape: S value Fitness Lethal:\t\t\t\t\t\t\t\t" << "'" << Gamma_Shape_Lethal << "' (Default)" << endl;
            logfile << "        - Gamma Scale: S value Fitness Lethal:\t\t\t\t\t\t\t\t" << "'" << Gamma_Scale_Lethal << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for LTHD!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            Normal_meanRelDom_Lethal = atof(temp.c_str()); parm[search].erase(0, pos + 1); Normal_varRelDom_Lethal = atof(parm[search].c_str());
            logfile << "        - Normal Mean: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom_Lethal << "'" << endl;
            logfile << "        - Normal Variance: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom_Lethal << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Normal_meanRelDom_Lethal = 0.05; Normal_varRelDom_Lethal = 0.1;
            logfile << "        - Normal Mean: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom_Lethal << "' (Default)" << endl;
            logfile << "        - Normal Variance: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom_Lethal << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for SUBA!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            Gamma_Shape_SubLethal = atof(temp.c_str()); parm[search].erase(0, pos + 1); Gamma_Scale_SubLethal = atof(parm[search].c_str());
            logfile << "        - Gamma Shape: S value Fitness Sub-Lethal:\t\t\t\t\t\t\t" << "'" << Gamma_Shape_SubLethal << "'" << endl;
            logfile << "        - Gamma Scale: S value Fitness Sub-Lethal:\t\t\t\t\t\t\t" << "'" << Gamma_Scale_SubLethal << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Gamma_Shape_SubLethal  = 0.1; Gamma_Scale_SubLethal = 0.2;
            logfile << "        - Gamma Shape: S value Fitness Sub-Lethal:\t\t\t\t\t\t\t" << "'" << Gamma_Shape_SubLethal << "' (Default)" << endl;
            logfile << "        - Gamma Scale: S value Fitness Sub-Lethal:\t\t\t\t\t\t\t" << "'" << Gamma_Scale_SubLethal << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for SUBD!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            Normal_meanRelDom_SubLethal = atof(temp.c_str()); parm[search].erase(0, pos + 1); Normal_varRelDom_SubLethal = atof(parm[search].c_str());
            logfile << "        - Normal Mean: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom_SubLethal << "'" << endl;
            logfile << "        - Normal Variance: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom_SubLethal << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Normal_meanRelDom_SubLethal = 0.3; Normal_varRelDom_SubLethal = 0.1;
            logfile << "        - Normal Mean: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_meanRelDom_SubLethal << "' (Default)" << endl;
            logfile << "        - Normal Variance: Dominance Fitness Lethal:\t\t\t\t\t\t\t" << "'" << Normal_varRelDom_SubLethal << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for COVAR!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos);
            proppleitropic = atof(temp.c_str()); parm[search].erase(0, pos + 1); genetic_correlation = atof(parm[search].c_str());
            logfile << "        - Proportion of QTL with Pleiotropic Fitness Effects :\t\t\t\t\t\t" << "'" << proppleitropic << "'" << endl;
            logfile << "        - Correlation (Rank) between QTL and Fitness Effects:\t\t\t\t\t\t" << "'" << genetic_correlation << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            proppleitropic = 0; genetic_correlation = 0;
            logfile << "        - Proportion of QTL with Pleiotropic Fitness Effects:\t\t\t\t\t\t" << "'" << proppleitropic << "' (Default)" << endl;
            logfile<<"        - Additive Genetic Correlation between QTL and Fitness Effects:\t\t\t\t\t"<<"'"<<genetic_correlation<<"' (Default)"<<endl;break;
        }
    }
    if(genetic_correlation < 0)
    {
        logfile << "        - Correlation can't be negative due to how effects are constructed. Alter Favorable direction!" << endl;
        exit (EXIT_FAILURE);
    }
    logfile << "    - Population Parameters:" << endl; search = 0;
    ne_founder = -10;
    while(1)
    {
        ...
        search++; if(search >= parm.size()){logfile<<endl<<"Couldn't find 'FOUNDER_Effective_Size:' variable in parameter file!"<<endl;exit(EXIT_FAILURE);}
    }
    if(ne_founder == -10 || ne_founder == 0)         /* will exit if 0 because can't be zero and if give a letter value will default to zero */
    {
        logfile << endl << "        - Wrong parameter for Founder Effective Population Size. Check Manual." << endl; exit (EXIT_FAILURE);
    }
    const int Ne = ne_founder;
    if(Ne != -5 && Ne_spec == ""){logfile << "        - Effective Population Size in Founders:\t\t\t\t\t\t\t" << "'" << Ne << "'" << endl;}
    if(Ne == -5 && Ne_spec != ""){logfile << "        - Effective Population Modeled from the scenario:\t\t\t\t\t\t" << "'" << Ne_spec << "'" << endl;}
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            if(pos == std::string::npos)
            {
                logfile << endl << "        - Should be two values not one for MUTATION!" << endl;
                exit (EXIT_FAILURE);
            }
            string temp = parm[search].substr(0,pos); u = strtod(temp.c_str(),NULL);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); PropQTL = atof(parm[search].c_str());
            logfile << "        - Mutation Rate:\t\t\t\t\t\t\t\t\t\t" << "'" << u << "'" << endl;
            logfile << "        - Proportion of mutations that can be QTL:\t\t\t\t\t\t\t" << "'" << PropQTL << "'" << endl; break;

        }
        search++; if(search >= parm.size())
        {
            u = 2.5e-8; PropQTL = 0.0;
            logfile << "        - Mutation Rate:\t\t\t\t\t\t\t\t\t\t" << "'" << u << "' (Default)" << endl;
            logfile << "        - Proportion of mutations that can be QTL:\t\t\t\t\t\t\t" << "'" << PropQTL << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); Variance_Additiveh2 = atof(parm[search].c_str());
            logfile << "        - Variance due to additive gene action (Phenotypic variance is 1.0):\t\t\t\t" << "'" << Variance_Additiveh2 << "'" << endl; break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'VARIANCE_A:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    if(Variance_Additiveh2 < 0 || Variance_Additiveh2 > 1.0)
    {
        logfile << endl << "VARIANCE_A outside of range (0.0 - 1.0)! Check parameter file!" << endl; exit (EXIT_FAILURE);
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); Variance_Dominanceh2 = atof(parm[search].c_str());
            logfile << "        - Variance due to dominant gene action (Phenotypic variance is 1.0):\t\t\t\t" << "'" << Variance_Dominanceh2 << "'" << endl; break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'VARIANCE_D:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    if(Variance_Dominanceh2 < 0 || Variance_Dominanceh2 > 1.0)
    {
        logfile << endl << "VARIANCE_D outside of range (0.0 - 1.0)! Check parameter file!" << endl; exit (EXIT_FAILURE);
    }
    logfile << "    - Selection and Mating Parameters" << endl;
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); GENERATIONS = atoi(parm[search].c_str());
            logfile << "        - Number of Generations to Simulate:\t\t\t\t\t\t\t\t" << "'" << GENERATIONS << "'" << endl; break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'GENERATIONS:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            DAMS = atoi(temp.c_str()); parm[search].erase(0, pos + 1); DamReplacement = atof(parm[search].c_str());
            logfile << "        - Number of Males in Population per Generation:\t\t\t\t\t\t\t" << "'" << SIRES << "'" << endl;
            logfile << "        - Sire Replacement Rate:\t\t\t\t\t\t\t\t\t" << "'" << SireReplacement << "'" << endl;
            logfile << "        - Number of Females in Population per Generation:\t\t\t\t\t\t" << "'" << DAMS << "'" << endl;
            logfile << "        - Dam Replacement Rate:\t\t\t\t\t\t\t\t\t\t" << "'" << DamReplacement << "'" << endl; break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'INDIVIDUALS:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    if(SireReplacement < 0 || SireReplacement > 1.0){logfile << endl << "Replacement rate outside of range (0.0 - 1.0). Check Parameterfile" << endl;}
    if(DamReplacement < 0 || DamReplacement > 1.0){logfile << endl << "Replacement rate outside of range (0.0 - 1.0). Check Parameterfile" << endl;}
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); fnd_haplo = atoi(parm[search].c_str()); break;
        }
        search++; if(search >= parm.size()){fnd_haplo = 2*(2*(DAMS+400)); break;}
    }
    const int FounderHaplotypes = fnd_haplo;
    logfile << "        - Founder Haplotypes:\t\t\t\t\t\t\t\t\t\t" << "'" << FounderHaplotypes << "'" << endl;
    int numberfoundersa = (DAMS+400) * 2;
    if(numberfoundersa > (FounderHaplotypes/2))
    {
        numberfoundersa = (FounderHaplotypes/2);
        logfile << "        - Had to reduce number of founder individuals created, may need to increase number of haplotypes created!." << endl;
    }
    const int numberfounders = numberfoundersa;
    logfile << "        - Number of Founder Individuals:\t\t\t\t\t\t\t\t" << "'" << numberfounders << "' (Based on Number of Dams)" << endl; search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 2); pos = parm[search].find(" ",0); string temp = parm[search].substr(0,pos);
            BetaDist_alpha = atof(temp.c_str()); parm[search].erase(0, pos + 1); BetaDist_beta = atof(parm[search].c_str());
            logfile << "        - Beta Alpha - distribution of mating pairs by age:\t\t\t\t\t\t" << "'" << BetaDist_alpha << "'" << endl;
            logfile << "        - Beta Beta - distribution of mating pairs by age:\t\t\t\t\t\t" << "'" << BetaDist_beta << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            BetaDist_alpha = 1.0; BetaDist_beta = 1.0;
            logfile << "        - Beta Alpha - distribution of mating pairs by age:\t\t\t\t\t\t" << "'" << BetaDist_alpha << "' (Default)" << endl;
            logfile << "        - Beta Beta - distribution of mating pairs by age:\t\t\t\t\t\t" << "'" << BetaDist_beta << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); OffspringPerMating = atoi(parm[search].c_str());
            logfile << "        - Number of offspring for each mating pair:\t\t\t\t\t\t\t" << "'" << OffspringPerMating << "'" << endl; break;
        }
        search++; if(search >= parm.size()){logfile << endl << "Couldn't find 'PROGENY:' variable in parameter file!" << endl; exit (EXIT_FAILURE);}
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 2); maxmating = atoi(parm[search].c_str());
            logfile << "        - Maximum Number of Full-Sibling taken per family:\t\t\t\t\t\t" << "'" << maxmating << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            maxmating = OffspringPerMating;
            logfile << "        - Maximum Number of Full-Sibling taken per family:\t\t\t\t\t\t" << "'" << maxmating << "'(Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 2); pos = parm[search].find(" ",0); string temp = parm[search].substr(0,pos);
            Selection = temp; parm[search].erase(0, pos + 1); SelectionDir = parm[search];
            logfile << "        - Selection Criteria:\t\t\t\t\t\t\t\t\t\t" << "'" << Selection << "'" << endl;
            logfile << "        - Selection Direction:\t\t\t\t\t\t\t\t\t\t" << "'" << SelectionDir << "'" << endl; break;
        }
        search++;
    ...
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 2); pos = parm[search].find(" ",0); string temp = parm[search].substr(0,pos);
            EBV_Calc = temp; parm[search].erase(0, pos + 1); pos = parm[search].find(" ",0); temp = parm[search].substr(0,pos);
            Solver = temp; parm[search].erase(0, pos + 1); Geno_Inverse = parm[search];
            logfile << "        - EBV estimated by:\t\t\t\t\t\t\t\t\t\t" << "'" << EBV_Calc << "'" << endl;
            logfile << "        - EBV solved by:\t\t\t\t\t\t\t\t\t\t" << "'" << Solver << "'" << endl;
            logfile << "        - If using genomic inverse calculated by:\t\t\t\t\t\t\t" << "'" << Geno_Inverse << "'" << endl; break;
        }
    ...
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 1);
            parm[search].erase(remove(parm[search].begin(), parm[search].end(), ' '), parm[search].end()); Mating = parm[search];
    ...
    while(1)
    {
        ...
        {
            size_t pos = parm[search].find(":", 0); parm[search].erase(0, pos + 2); pos = parm[search].find(" ",0); string temp = parm[search].substr(0,pos);
            Culling = temp; parm[search].erase(0, pos + 1); MaximumAge = atoi(parm[search].c_str());
            logfile << "        - Culling Criteria:\t\t\t\t\t\t\t\t\t\t" << "'" << Culling << "'" << endl;
    ...
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            logfile << "        - LD Parameters by Generation Created:\t\t\t\t\t\t\t\t" << "'" << Create_LD_Decay << "'" << endl; break;
        }
        search++;
        if(search >= parm.size())
        {
            Create_LD_Decay = "no";
            logfile << "        - LD Parameters not by Generation Created:\t\t\t\t\t\t\t\t" << "'" << Create_LD_Decay << "' (Default)" << endl; break;
        }
    }
    search = 0;
    while(1)
    {
        ...
        if(fnd!=std::string::npos)
        {
            ...
            OutputGeno = temp;
            if(OutputGeno == "no"){outputgeneration = 0; logfile << "        - No genotypes are kept and placed in file." << endl;}
            if(OutputGeno == "yes")
            {
                parm[search].erase(0, pos + 1); outputgeneration = atoi(parm[search].c_str());
                logfile << "        - Genotypes after generation " << outputgeneration << " are kept and placed in file." << endl;
            }
            break;
        }
        search++;
        if(search >= parm.size()){OutputGeno="yes";outputgeneration=0;logfile<<"        - All genotypes are kept and placed in file. (Default)"<<endl;break;}
    }
    logfile << "Parameters Read in!!" << endl;
    parm.clear();
    ...
    {
        logfile << endl << "Cannot have a genetic correlation between fitness and quantitative QTL when no sublethal QTL!" << endl;
        exit (EXIT_FAILURE);
    }
    if(numQTL > 0 && Variance_Additiveh2 == 0.0 && Variance_Dominanceh2 == 0.0)
    {
        logfile << endl << "No additive or dominance variance so can't have quantitative QTL be greater than 0!" << endl;
        exit (EXIT_FAILURE);
        
    }
    if(numQTL == 0 && Variance_Additiveh2 == 0.0 && Variance_Dominanceh2 == 0.0 && Selection != "random")
    {
        logfile << endl << "Cannot have selection occuring if no additive or dominance variance!" << endl;
        exit (EXIT_FAILURE);
    }
    if(Variance_Additiveh2 == 0.0 && Variance_Dominanceh2 > 0.0)
    {
        logfile << endl << "Cannot have dominance variance, but no additive variance!" << endl;
        exit (EXIT_FAILURE);
    }
        ...
        if(Ne == -5 && Ne_spec != "")
        {
            if(Ne_spec == "CustomNe")
            {
                ...
                for(int i = 0; i < numChr; i++)
                {
                    ...
                    logfile << "    - Finished Generating Sequency Information for Chromosome " << i + 1 << endl;
                }
            }
            if(Ne_spec == "Ne70")
            {
    ...
    if(StartSim == "founder" || StartSim == "sequence")
    {
        ...
        for(int i = 0; i < numberfounders; i++)
        {
            for (int j = 0; j < 2; j++)                                                         /* Loop through 2x to get two haplotypes */
            {
                while(1)
                {
                    ...
                    vector < double > temp;
                    vector < int > tempa;
                    temp.push_back(distribution(gen) * FounderHaplotypes);
                    temp[0] = temp[0] + 0.5;
                    tempa.push_back(temp[0]);
                    if(haplotype_used[temp[0]] == 0)                                                /* first checks to make sure hasn't been used */
                    {
                        haplotype_used[tempa[0]] = 1;
                        if(tempa[0] == 0){tempa[0] = 1;}
                        linenumber[i*2+j] = tempa[0];
                        temp.clear(); tempa.clear();
                        break;
                    }
                    temp.clear();
                }
        ...
        for(int i = 0; i < SNPFreq.size(); i++){SNPFreq[i] = SNPFreq[i] / (2 * numberfounders); output15 << SNPFreq[i] << " ";}
        output15.close();
        ...
        {
            output << population_QTL[i].getLocation() << " " << population_QTL[i].getAdditiveEffect() << " "<< population_QTL[i].getDominanceEffect() << " " << population_QTL[i].getType() << " "<< population_QTL[i].getGenOccured() << " " << population_QTL[i].getFreq() << endl;
        }
        output.close();
        ...
        ////////////////////////////////////////////////////////////////////////////////////
        //** Create a vector to store EBVs of all the animals in order to utilize as starting values **/
        ...
        for(int Gen = 1; Gen < (GENERATIONS + 1); Gen++)
        {
            ...
            if(scalefactaddh2 != 0)
            {
                ...
                double* estimatedsolutions = new double[LHSsize];
                ...
                if(Solver == "pcg")                                                                         /* Solve equations using pcg */
                {
                    logfile << "           - Starting " << Solver << "." << endl;
                    start = time(0);
                    int* solvedatiteration = new int[1];
            ...
            {
                if(MarkerIndex[m_counter] == i){FullMap[i] = MarkerMapPosition[m_counter]; m_counter++;}    /* If is marker put in full map position */
                if(QTL_Index[qtl_counter] == i){FullMap[i] = QTL_MapPosition[qtl_counter]; qtl_counter++;}  /* If is quantitative QTL put in full map position */
            }
        ...
        clock_t start;
        clock_t end;
        double* estimatedsolutions = new double[(TotalAnimalNumber + 1)];
        for(int i = 0; i < (TotalAnimalNumber + 1); i++){estimatedsolutions[i] = 0;}
        if(scalefactaddh2 != 0)
        {
        ...
        cout << "Simulation has completed normally (Took: " << difftime(fullend_time,fullbegin_time) / 60 << " minutes)" << endl << endl;
    }
}